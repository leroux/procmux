...
is disconnected.  Zero intelligence — no knowledge of any higher-level
protocol, framework, or application layer.
"""
...
class ProcmuxServer:
    ...
    async def _cmd_spawn(self, msg: CmdMsg):
        ...
        # Build environment: inherit parent env by default, merge overrides
        if msg.env_inherit:
            spawn_env = {**os.environ, **msg.env} if msg.env else None
        else:
            spawn_env = msg.env if msg.env else {}
    ...
    async def _cmd_list(self):
        ...
        await self._send_result(ResultMsg(ok=True, processes=procs))
    ...
    async def _kill_process(self, mp: ManagedProcess):
        ...
        to SIGKILL after 5 seconds.  Uses os.killpg() so the entire process
