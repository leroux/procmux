...
async fn list_and_status() {
    ...
    assert!(list.ok);
    let processes = list.processes.unwrap();
    assert!(processes.as_object().unwrap().is_empty());
    ...
    assert!(list.ok);
    let processes = list.processes.unwrap();
    let processes = processes.as_object().unwrap();
    assert_eq!(processes.len(), 1);
    assert!(processes.contains_key("test-sleep"));
    assert_eq!(processes["test-sleep"]["status"], "running");
