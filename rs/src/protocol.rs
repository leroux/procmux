...
mod tests {
    ...
    #[test]
    fn parse_cmd_spawn() {
        let json = r#"{"type":"cmd","cmd":"spawn","name":"worker-1","cli_args":["python","run.py"],"env":{"HOME":"/home/test"},"cwd":"/tmp"}"#;
        let msg: ClientMsg = serde_json::from_str(json).unwrap();
        match msg {
            ClientMsg::Cmd(cmd) => {
                assert_eq!(cmd.cmd, "spawn");
                assert_eq!(cmd.name, "worker-1");
                assert_eq!(cmd.cli_args, vec!["python", "run.py"]);
            }
    ...
    #[test]
    fn parse_stdin() {
        let json = r#"{"type":"stdin","name":"worker-1","data":{"type":"user","content":"hello"}}"#;
        let msg: ClientMsg = serde_json::from_str(json).unwrap();
        match msg {
            ClientMsg::Stdin(stdin) => {
                assert_eq!(stdin.name, "worker-1");
    ...
    #[test]
    fn serialize_result() {
        let msg = ServerMsg::Result(ResultMsg::ok("worker-1"));
